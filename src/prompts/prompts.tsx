import { Terrain } from '@rsodre/crawler-data'
...
When generating an NPC, you must generate its "behaviour_mode" based on the provided "gem_type", as listed in the "Behaviour modes for NPCs" list below.
...
{
  "npc": {
    "npc_name": "Tremor, the Stone Guardian",
    "npc_description": "A massive golem made of the very rocks of the cavern. It guards the caverns fiercely, attacking any who disturb the peace.",
    "behaviour_mode": "A monster NPC who is hostile",
    "npc_quirk": "Tremor is sensitive to loud noises. A loud enough sound can stun it momentarily."
  }
}
...
* quest (when gem_type is silver): A friendly NPC who needs help
* mystery (when gem_type is gold): An empty room that presents a mystery or challenge
* fight (when gem_type is saphire): A monster NPC who is hostile
* trickster (when gem_type is emerald): A trickster NPC who is deceptive
* trial (when gem_type is ruby): A grand mythical beast who tests if the player is worthy
* artefact (when gem_type is diamond): A magical artifact
* boss (when gem_type is ethernite): The world boss from the world description
* key (when gem_type is kao): Solve a challenge to get the key to beating the world boss
